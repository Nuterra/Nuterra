...
namespace Maritaria
{
	public class SplashScreenHandler : MonoBehaviour
	{
		...
		public string _contentFormat = "<color=#eeeeee><size=20>Mod by <color=#{0}>Maritaria</color></size></color>";
		...
		public void OnGUI()
		{
			if (CanDraw())
			{
				_content.text = string.Format(_contentFormat, GetFormattedColor(GetRainbowColor()));
		...
		{
			return Color.HSVToRGB(Mathf.Sin(Time.time) * 0.5f + 0.5f, 1f, 1f);
		}
